...
def main(savePath, load_path, log_weight, lr=1e-3, weight_decay=0, total_epoch=100):
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 2:
        ...
        log_weight['albedo'] = float(sys.argv[6])
        log_weight['shading'] = float(sys.argv[7])
        log_weight['normal'] = float(sys.argv[8])
        log_weight['image'] = float(sys.argv[9])
        main(savePath, load_path, log_weight, lr, weight_decay, total_epoch)
    else:
        main(savePath)
